...
| **Release**       | `release merge-dev`                                                     | Merge `origin/dev` into open regular release branches in a scratch worktree, then push atomically (`--dry-run`, `--versions`). Hotfixes (PRs to `main`), title/base mismatches and heads with PRs to both bases are listed as `skipped` with a reason. `--keep-conflicts` hands a real conflict to a CLI-owned resolution worktree; `--continue`/`--abort` resume or drop that hand-off. `/infra-kit:merge-dev` is the agent entry point for the whole flow |
|                   | `release list`                                                          | List available releases                                                                                                                                                                                                                                                                                                                                                                                                                                     |
|                   | `release create`                                                        | Create a new release; `-r <token>[@yyyy-mm-dd][:type[:description]]` — the `@date` is the Jira fix version’s planned release date                                                                                                                                                                                                                                                                                                                           |
|                   | `release edit`                                                          | Edit a release’s Jira description and/or planned release date (`--description`, `--release-date`)                                                                                                                                                                                                                                                                                                                                                           |
...
| **Setup**         | `infra-kit setup [--tools <ids...>] [--update [ids...]] [--skip-tools]` | Set up infra-kit on this machine: shell integration, the Claude Code plugin, the `.mcp.json` key, then install or update the external CLIs (brew, git, aws, gh, doppler, portless). `--tools` / `--update` narrow the converge; `--skip-tools` writes only, reporting what is missing without installing it. Written binary-qualified here because `pnpm setup` and `pnpm run setup` both exist in every consumer and both mutate                           |
...
infra-kit dev --no-watch   # Build once; don't rebuild & restart on save (watch is on by default)
