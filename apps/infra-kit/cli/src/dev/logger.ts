...
import type { IConfigLogger, ILogger, ITraceInfo, LogLevel, LoggerLevelValue } from './interfaces.js'
import { LOG_LEVEL_TO_METHOD, LoggerLevel } from './interfaces.js'

/** Accepted extra-argument types across the Powertools logger method signatures. */
type ExtraInput = string | Error | object
...
const getRandomHexStringId = (strlen: number): string => {
  return `0x${[...new Array<number>(strlen)]
    .map(() => {
...
export class Logger implements ILogger {
  ...
  public trace = (input: string | object, ...extraInput: ExtraInput[]): void => {
  ...
  public getByLevel(level: LoggerLevelValue): (input: string | object, ...extraInput: ExtraInput[]) => void {
  ...
  public debug(input: string | object, ...extraInput: ExtraInput[]): void {
  ...
  public info(input: string | object, ...extraInput: ExtraInput[]): void {
    const message = typeof input === 'string' ? input : JSON.stringify(input)
    const extraData = extraInput.length > 0 ? (extraInput[0] as object) : undefined
  ...
  public warn(input: string | object, ...extraInput: ExtraInput[]): void {
    const message = typeof input === 'string' ? input : JSON.stringify(input)
    const extraData = extraInput.length > 0 ? (extraInput[0] as object) : undefined
  ...
  public error(input: string | object, ...extraInput: ExtraInput[]): void {
    const message = typeof input === 'string' ? input : JSON.stringify(input)
    const extraData = extraInput.length > 0 ? (extraInput[0] as object) : undefined
  ...
  public fatal(input: string | object, ...extraInput: ExtraInput[]): void {
    const message = typeof input === 'string' ? input : JSON.stringify(input)
    const extraData = extraInput.length > 0 ? (extraInput[0] as object) : undefined
  ...
  public critical(input: string | object, ...extraInput: ExtraInput[]): void {
  ...
  public injectLambdaContext(
    ...
    return (_target: unknown, _propertyKey: string | symbol, _descriptor: PropertyDescriptor) => {
      return _descriptor
    }
  }
  ...
  /** Delete every accumulated persistent log attribute in place. */
  private clearPersistentKeys(): void {
  ...
  private log(pLevel: ILoggerLevelConfig | undefined, message: string, extraData?: object, pPrint = false): void {
    ...
    process.stdout.write(`${level.chalk(this.buildOutput(message, level, extraData))}\n`)
  }
}
