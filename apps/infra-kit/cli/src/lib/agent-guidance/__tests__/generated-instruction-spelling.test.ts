...
const commandSpellings = (markdown: string): string[] => {
  ...
  // names are MCP tool identities (`mcp__infra-kit__setup`), never shell argv — the
  // ambiguity with `pnpm run setup` cannot be reached from a tool name, and writing
  // `infra-kit setup` there would name a tool that does not exist.
  let exempt = false
