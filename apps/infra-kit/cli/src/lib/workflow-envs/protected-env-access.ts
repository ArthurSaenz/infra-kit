...
//
// `isMcpMode()` is called INSIDE this function on purpose. `mcpMode` is a mutable object read at
// call time, and the flag is assigned during MCP server bootstrap (`mcp/server.ts`), which happens
// after this module is imported. Hoisting the read to module scope — `const inMcp =
// mcpMode.enabled` at the top — would freeze `false` and silently degrade `'cli-only'` into
// `'allow'` for every agent.
