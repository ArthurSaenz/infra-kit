...
import { check, checkMcpTool } from '../check'

const tmpDirs: string[] = []

const makeTmpPackage = (config: string, packageJson: Record<string, unknown>): string => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'check-cmd-'))
...
describe('mCP tool registration', () => {
  it('exposes the canonical `check` tool', () => {
    expect(checkMcpTool.name).toBe('check')
  })
})
