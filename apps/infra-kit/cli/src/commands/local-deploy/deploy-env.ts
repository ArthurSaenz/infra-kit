 ...
 *
 * Sets the `VITE_*` contract the workflow YAML would otherwise supply, and STRIPS every ambient
 * `VITE_*` the developer's shell carries so nothing leaks into the artifact. `DEPLOY_*` are set
 * alongside for forward compatibility; nothing reads them today.
 *
 * @example
...
// bundle deployed to a dev environment links to PRODUCTION. Silently, with a green deploy. That
// gap is why `infra-kit local deploy-all` exists rather than a README telling people which vars
// to export.
//
// The strip is the other half. Builds run `turbo … --env-mode=loose`, which passes the entire
// ambient environment through, and `turbo.json` declares `build.env: ["VITE_*"]`. After
// `infra-kit env-load` a developer's shell carries Doppler values — so without stripping, a local
// deploy would bake whatever `VITE_*` happens to be exported into an artifact shipped to a shared
// environment.
