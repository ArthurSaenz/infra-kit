...
describe('section coverage', () => {
  ...
  it('covers exactly 32 checks', () => {
    // 32, up from 31: `portless service target` joined the proxy rows.
    expect(DOCTOR_CHECK_NAMES).toHaveLength(32)
    expect(new Set(DOCTOR_CHECK_NAMES).size).toBe(32)
  })
