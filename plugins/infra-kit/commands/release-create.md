...
If that resource cannot be read — this session may expose no resource tools, or the server may predate it: it needs infra-kit 0.5.0 or newer — call `mcp__plugin_infra-kit_infra-kit__release-create` directly — or `mcp__infra-kit__release-create` if this repo's `.mcp.json` still registers the server; if neither exists the plugin predates 0.8.0 — say so and stop — and let its confirm gate drive the rest.
