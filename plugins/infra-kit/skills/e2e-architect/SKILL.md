...
Codifies the per-feature Playwright e2e convention proven in this monorepo family's
backoffice/client test suites. Each feature gets a folder under `src/tests/<feature>/` containing
one Page Object (the only place selectors live), one fixture that guarantees cleanup even on
mid-test failure, and a set of specs split by behavioral axis. The canonical reference is
