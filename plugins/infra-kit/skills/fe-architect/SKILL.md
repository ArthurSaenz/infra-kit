...
3. Scaffold the feature — this substitutes every placeholder, renames the files, picks one services
   ...
   ```bash
   node "${CLAUDE_PLUGIN_ROOT}"/skills/fe-architect/scripts/scaffold_feature.mjs [target-features-dir] [feature-name]
...
node "${CLAUDE_PLUGIN_ROOT}"/skills/fe-architect/scripts/validate_feature.mjs [features-path]/[feature-name]
...
node "${CLAUDE_PLUGIN_ROOT}"/skills/fe-architect/scripts/check_structure.mjs [features-path]/[feature-name]
```
...
The scripts are themselves covered by `pnpm run test:claude`, run from the infra-kit repo root. If
you change one, run that suite — nothing else will.
