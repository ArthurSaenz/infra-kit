...
import { errors, fixture, matches, runScript, warnings } from './helpers.mjs'
...
describe('validate_feature', () => {
  ...
  describe('P0-1: atom classification', () => {
    test('a derived atom directly above a write-only Fx atom is not flagged', async () => {
      ...
      assert.ok(!matches(errors(stdout), '$hasData'), `derived atom was flagged: ${JSON.stringify(errors(stdout))}`)
  ...
  describe('P0-2: type-only imports', () => {
    test('an inline `import { type X }` is not a cross-feature violation', async () => {
      const { code, stdout } = await runScript('validate_feature.mjs', [fixture('imports', 'consumer')])
  ...
  describe('P0-3: className resolution', () => {
    test('className declared in types.ts satisfies rule 6', async () => {
      ...
      assert.ok(!matches(errors(stdout), 'className'), `false className error: ${JSON.stringify(errors(stdout))}`)
      assert.ok(!matches(warnings(stdout), 'className'), `false className warning: ${JSON.stringify(warnings(stdout))}`)
      assert.equal(code, 0)
    })
  ...
  describe('acronym-tolerant component export names', () => {
    test('a component exported under its conventional acronym casing (AIChatContainer) is not flagged', async () => {
      const { code, stdout } = await validate('acronym-export')

      assert.ok(
        !matches(errors(stdout), 'should export'),
        `acronym export was flagged: ${JSON.stringify(errors(stdout))}`,
      )
      assert.deepEqual(errors(stdout), [], 'expected no errors')
      assert.deepEqual(warnings(stdout), [], 'expected no warnings')
      assert.equal(code, 0)
    ...
    test('a component exported under a name that still does not match, case-insensitively, still errors', async () => {
      const dir = await mkdtemp(join(tmpdir(), 'fe-architect-'))

      try {
        const feature = join(dir, 'acronym-export')
        await cp(fixture('acronym-export'), feature, { recursive: true })
        await writeFile(join(feature, 'containers', 'ai-chat-container.tsx'), 'export const WrongName = () => null\n')

        const { code, stdout } = await runScript('validate_feature.mjs', [feature])

        assert.ok(
          matches(errors(stdout), "should export 'AiChatContainer'", "found: 'WrongName'"),
          `mismatch was not flagged: ${JSON.stringify(errors(stdout))}`,
        )
        assert.equal(code, 1)
      } finally {
