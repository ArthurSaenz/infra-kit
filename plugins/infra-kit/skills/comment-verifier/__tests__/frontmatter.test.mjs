...
// YAML parser. Two failure modes are silent and cost the whole skill: a block-form value parses to
...
test('no field is block form, which parses to an empty value', () => {
  // Asserted over every key rather than a named list field: the plugin frontmatter no longer carries
  // one, and a test naming a key that has been dropped passes on `undefined` without checking
  // anything. A block-form list or an empty folded scalar is what produces the empty value here.
  for (const [key, value] of Object.entries(parseFrontmatterLikeOmc(SKILL_MD))) {
    assert.notEqual(value, '', `${key} parsed to empty — write it inline, not in block form`)
  }
})
