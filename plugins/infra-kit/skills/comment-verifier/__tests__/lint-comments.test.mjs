...
import { fixture, REPO_ROOT, repoPath, SCRIPT, SKILL_DIR } from './paths.mjs'
...
test('the block hidden by that regex is found in whole-file source', () => {
  ...
  const file = fixture('regex-hides-block.ts')
...
test('a run with no flags reports the scope and the rung that answered', () => {
  const stdout = execFileSync(
    process.execPath,
    [SCRIPT, fixture('clean.ts')],
    { cwd: REPO_ROOT, encoding: 'utf8' },
  )
  const report = JSON.parse(stdout)

  assert.deepEqual(report.scope, [fixture('clean.ts')])
...
test('named arguments are honoured verbatim, and the unjudgeable ones are reported', () => {
  ...
  const named = [
    repoPath(SKILL_DIR, 'SKILL.md'),
    fixture('__tests__', 'scoped-out.ts'),
    'does/not/exist.ts',
  ]
