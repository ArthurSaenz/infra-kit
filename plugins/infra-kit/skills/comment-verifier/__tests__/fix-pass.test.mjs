...
import { fixture, fixturesPorcelain, REPO_ROOT, SCRIPT } from './paths.mjs'
...
test('--snapshot and --verify-fix agree on a real edit, and exit non-zero on a bad one', () => {
  const target = fixture('restating-fix.ts')
...
test('verification covers everything the snapshot captured, not a re-guessed subset', () => {
  ...
  const dir = fixture('manifest-probe')
...
test('the CLI refuses a verdict naming a file the snapshot never captured', () => {
  const dir = fixture('coverage-probe')
...
test('--snapshot on a path that is not a readable file refuses with a message, not a stack', () => {
  ...
  try {
    for (const target of ['does/not/exist.ts', fixture()]) {
...
test('--snapshot resolving to nothing refuses rather than writing an empty baseline', () => {
  ...
  const scratch = mkdtempSync(join(tmpdir(), 'comment-verifier-'))
  const dir = fixture('empty-scope-probe')
...
test('a snapshot whose copy fails still fails closed, because the manifest is written first', () => {
  const dir = fixture('manifest-order-probe')
...
test('a review-mode run leaves the scoped files byte-identical', () => {
  ...
  // rather than this script. The property under test is that a review-mode run writes nothing, so
  // the bytes of the scoped files say so directly and the porcelain call is scoped to the fixture
  // tree — the only region a run of this scope could dirty. `tree-hygiene.test.mjs` proves that
  // pathspec is live; an unmatched one would exit 0 with empty output and compare empty to empty.
  const scope = [fixture('restating.ts'), fixture('clean.ts'), fixture('restating-fix.ts')]
  const bytes = () => scope.map((file) => readFileSync(join(REPO_ROOT, file), 'utf8'))

  const beforeBytes = bytes()
  const beforeStatus = fixturesPorcelain()
  runScript(scope)
  assert.deepEqual(bytes(), beforeBytes)
  assert.equal(fixturesPorcelain(), beforeStatus)
})
