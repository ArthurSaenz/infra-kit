...
```
node "${CLAUDE_PLUGIN_ROOT}"/skills/comment-verifier/scripts/lint-comments.mjs --hash '// Ask for confirmation'
```
...
```
node "${CLAUDE_PLUGIN_ROOT}"/skills/comment-verifier/scripts/lint-comments.mjs --verify-fix <verdict.json> --baseline <dir>
```
