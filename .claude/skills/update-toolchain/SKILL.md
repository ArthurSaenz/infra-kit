...
contract consumed by the 9 `infra-kit.config.ts` files under `vendor/configs/*` and
`vendor/packages/*`. infra-kit is pre-1.0 (0.x), so a minor/patch bump may carry breaking changes.
...
> does not have) — so **`--all` and `--root` never reach the vendor configs**. To exercise the 9
> config files, run a **bare `pnpm exec infra-kit audit` from inside each config directory**. The
> 9 directories are:
> `vendor/configs/{prettier-config,ts-config,vitest-config,serverless-config,eslint-config,storybook-config}`
> and `vendor/packages/{docs-ui,web-toolkit,lib-be-dev}`.
> With `--json`, each directory's audit returns a `checks` array; the config-contract signal is the
...
4. Establish a BASELINE at the OLD version: for each of the 9 config directories listed above, run
...
9. HARD GATE — verify the config contract still loads under the new version. Re-audit each of the 9
   config directories with `pnpm exec infra-kit audit --json` run from inside each directory (same
   ...
   - Require positive proof of loading: confirm every one of the 9 directories returns an
