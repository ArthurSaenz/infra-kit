...
const deny = (reason) => {
  denyDecision(
    [
      ...
      '',
      '`infra-kit` not on PATH in this session? In a terminal: `infra-kit setup` installs it,',
      'then restart Claude Code at the repo root.',
      '',
      'Deploying to a non-prod environment IS allowed — but it goes through infra-kit,',
      'which enforces the deploy rules:',
      '    pnpm exec infra-kit release-deploy-all         # dev / stage / personal envs',
      '    pnpm exec infra-kit release-deploy-selected    # a subset of services',
      '',
